...
#         return pas
    ...
    def loss_adj(self, Xs, criterion, halfAs):
        inputs = []
        labels = []
        for i, X in enumerate(Xs):
            pdists = torch.pdist(torch.FloatTensor(X).to(device)).view(-1, 1)
            pos_inds = torch.where(halfAs[i] == 1)[0]
            neg_inds = torch.where(halfAs[i] == 0)[0]
            selected_inds = np.random.permutation(32)
#             print(selected_inds)
            inds = torch.cat([pos_inds[selected_inds], neg_inds[selected_inds]], dim=0)
            input = self.prepare_input(pdists, inds=inds)
            inputs.append(input)
            label = halfAs[i][inds]
            labels.append(label)
        inputs = torch.cat(inputs, dim=0)
        labels = torch.cat(labels, dim=0)
        preds = self.forward_inputs(inputs).view(-1, 2)
...
print(model_gnn)
# optim = torch.optim.Adam(chain(model1.parameters(),
# model_gnn.parameters()), lr=0.01) # weight_decay=5e-4
...
for iter in range(2000):
    ...
    batch_halfas = torch.stack(
        [torch.LongTensor(A_to_halfA(x)).to(device) for x in batch_as])
    loss_adj = model1.loss_adj(batch_xs, loss_fn, batch_halfas)
...
#     pred_As to
    pred_adjs = batch_pdist_to_adjs(pred_As[:, :, 1])
    if iter % 100 == 0:
        print(pred_As[0, :10])
    # A to graphsage
...
#     loss_shuffle = loss_shuffle_features(batch_xs[:4])
    loss_distance = 0
    loss_shuffle = 0
    loss = loss_gnn + loss_adj  # + loss_shuffle*0.1

...
def save_all_validation_graphs(args, train_graphs, test_graphs, epoch):
    ...
    if args.gen_multigraph:
        ...
        save_graphs(Asbm, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/Atrain-Xtrain")

        print('AtrainF-Xtrain')
        print(f"Number of edges: {Af.sum()}")
        save_graphs(Af, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/AtrainF-Xtrain")

        print('A2-Xtest')  # A2 = Af
        Af, X, L, _ = test_graphs[ind_test]
        save_graphs(Af, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/A2-Xtest")

        print('A3-Xtest')  # A3 = KNN(X)
        _, X, L, _ = test_graphs[ind_test]
        A3 = generate_graph(torch.FloatTensor(X), kind="knn", k=k)
        save_graphs(A3, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/A3-Xtest")

        print('A4-Xtest')  # A4 = Sigmoid(X)
        _, X, L, _ = test_graphs[ind_test]
        A4 = generate_graph(torch.FloatTensor(X), kind="sigmoid", k=k)
        save_graphs(A4, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/A4-Xtest")

        print('Atest-Xtest')  # A4 = Sigmoid(X)
        _, X, L, Asbm = test_graphs[ind_test]
        save_graphs(Asbm, X, L, f"{args.data_savepath}/synf-seed{args.seed}-epoch{epoch}/Atest-Xtest")
...
#         
    ...
    def predict_adj(self, features):
        ...
        diff = torch.FloatTensor(features[inds[0]] - features[inds[1]]).unsqueeze(0).cuda()
        log_theta = self.output_theta(diff) 
        log_alpha = self.output_alpha(diff)  # B X (tt+K)K
...
def mixture_bernoulli_loss(label, log_theta, log_alpha, adj_loss_func):
    ...
    """
    K = log_theta.shape[1]
    reduce_adj_loss = torch.stack([adj_loss_func(log_theta[:, kk], label) for kk in range(K)], dim=1)
